...
{
    [Target(MoreNLogTargetTypeName)]
    public class MoreNLogInput : TargetWithContext, IObservable<EventData>
    {
        internal const string MoreNLogTargetTypeName = "EventFlowInput";
        ...
        public MoreNLogInput(IHealthReporter healthReporter)
        {
        ...
        private EventData ToEventData(LogEventInfo logEvent)
        {
            ...
            {
                NLog.Common.InternalLogger.Warn(ex, MoreNLogTargetTypeName + "(Name={0}): Failed to render event providername", Name);
                _healthReporter.ReportWarning($"{nameof(MoreNLogInput)}: event providername could not be rendered{Environment.NewLine}{ex.ToString()}");
            }

            if (string.IsNullOrEmpty(providerName))
                providerName = string.IsNullOrEmpty(logEvent.LoggerName) ? nameof(MoreNLogInput) : logEvent.LoggerName;
            ...
            if (logEvent.Exception != null)
            {
                ...
                eventData.AddPayloadProperty("Exception", logEvent.Exception, _healthReporter, nameof(MoreNLogInput));
            }
            ...
            try
            {
                eventData.AddPayloadProperty("Message", RenderLogEvent(Layout, logEvent), _healthReporter, nameof(MoreNLogInput));
            }
            catch (Exception ex)
            {
                NLog.Common.InternalLogger.Warn(ex, MoreNLogTargetTypeName + "(Name={0}): Failed to render event message", Name);
                _healthReporter.ReportWarning($"{nameof(MoreNLogInput)}: event message could not be rendered{Environment.NewLine}{ex.ToString()}");
                eventData.AddPayloadProperty("Message", logEvent.FormattedMessage, _healthReporter, nameof(MoreNLogInput));
            }
        ...
        private void CaptureLogEventContextProperties(IDictionary<string, object> properties, EventData eventData)
        {
            if (properties != null)
            {
                foreach (var property in properties)
                {
                    try
                    {
                        eventData.AddPayloadProperty(property.Key, property.Value, _healthReporter, nameof(MoreNLogInput));
                    }
                    catch (Exception ex)
                    {
                        NLog.Common.InternalLogger.Warn(ex, MoreNLogTargetTypeName + "(Name={0}): Failed to render event context property: {1}", Name, property.Key);
                        _healthReporter.ReportWarning($"{nameof(MoreNLogInput)}: event context property '{property.Key}' could not be rendered{Environment.NewLine}{ex.ToString()}");
                    }
                }
            }
        }
        ...
        private void CaptureLogEventPropertiesEventData(IDictionary<object, object> properties, EventData eventData)
        {
            foreach (var property in properties)
            {
                ...
                try
                {
                    ...
                    eventData.AddPayloadProperty(propertyKey, ToRawValue(property.Value), _healthReporter, nameof(MoreNLogInput));
                }
                catch (Exception ex)
                {
                    NLog.Common.InternalLogger.Warn(ex, MoreNLogTargetTypeName + "(Name={0}): Failed to render event property: {1}", Name, propertyKey);
                    _healthReporter.ReportWarning($"{nameof(MoreNLogInput)}: event property '{propertyKey}' could not be rendered{Environment.NewLine}{ex.ToString()}");
                }
            }
        }
        ...
        private void CaptureTargetContextEventData(LogEventInfo logEvent, EventData eventData)
        {
            for (int i = 0; i < ContextProperties.Count; ++i)
            {
                ...
                try
                {
                    eventData.AddPayloadProperty(property.Name, RenderLogEvent(property.Layout, logEvent), _healthReporter, nameof(MoreNLogInput));
                }
                catch (Exception ex)
                {
                    NLog.Common.InternalLogger.Warn(ex, MoreNLogTargetTypeName + "(Name={0}): Failed to render event target property: {1}", Name, property.Name);
                    _healthReporter.ReportWarning($"{nameof(MoreNLogInput)}: event target property '{property.Name}' could not be rendered{Environment.NewLine}{ex.ToString()}");
                }
            }
        }
        ...
        protected override bool SerializeItemValue(LogEventInfo logEvent, string name, object value, out object serializedValue)
        {
            ...
            {
                NLog.Common.InternalLogger.Warn(ex, MoreNLogTargetTypeName + "(Name={0}): Failed to render event property: {1}", Name, name);
                _healthReporter.ReportWarning($"{nameof(MoreNLogInput)}: event property '{name}' could not be rendered{Environment.NewLine}{ex.ToString()}");
                serializedValue = null;
                return false;
            }
        }
